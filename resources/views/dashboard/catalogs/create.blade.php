...
@section('container')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h4>Buat Katalog Baru</h4>
    </div>

    <div class="col-md-8">
        <form action="/dashboard/catalogs" method="POST" class="mb-5" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
                <label for="title" class="form-label">Produk dan Harga dari Supplier</label>
                <select class="form-select" name="id_barang">
                    @foreach ($suppliers as $supplier)
                        @if (old('id_barang') == $supplier->id)
                            <option value="{{ $supplier->id }}" selected>{{ $supplier->nama_barang }}, Rp.
                                <?= number_format($supplier->harga_pasok, 0, ',', '.') ?></option>
                        @else
                            <option value="{{ $supplier->id }}">{{ $supplier->nama_barang }}, Rp.
                                <?= number_format($supplier->harga_pasok, 0, ',', '.') ?>
                                /{{ $supplier->satuan->nama_satuan }} (per {{ $supplier->updated_at->format('d M Y') }})
                            </option>
                        @endif
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('nama_barang') is-invalid  @enderror" id="nama_barang"
                    name="nama_barang" required autofocus value="{{ old('nama_barang') }}">
                @error('nama_barang')
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('harga_barang') is-invalid  @enderror" id="harga_barang"
                    name="harga_barang" required autofocus value="{{ old('harga_barang') }}">
                @error('harga_barang')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                <label for="desk_barang" pattern="[a-zA-Z0-9-]+" class="form-label">Deskripsi Produk</label>
                <input id="desk_barang" type="hidden" name="desk_barang" value="{{ old('desk_barang') }}">
