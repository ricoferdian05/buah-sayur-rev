    ...
    <div class="col-md-8">
        <form action="/dashboard/catalogs" method="POST" class="mb-5" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="id_barang">
                  @foreach ($suppliers as $supplier)
                      ...
                      @else
                          <option value="{{ $supplier->id }}">{{ $supplier->nama_barang }}, Rp. <?= number_format($supplier->harga_pasok, 0, ',', '.') ?> /{{ $supplier->satuan->nama_satuan }} (per {{ $supplier->updated_at->format('d M Y') }})</option>
                      @endif
                  @endforeach
              </select>
              </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('harga_barang') is-invalid  @enderror" id="harga_barang" name="harga_barang" required autofocus value="{{ old('harga_barang') }}">
                @error('harga_barang')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
